...
class Timer {
  time_t now_c;
  int wait_time; // the time to wait between updates. in milliseconds.
                 // recommended to set it above 60000 (60 sec).
...
public:
  ...
  void wait_next() {
    Sleep(wait_time);
    LOG("Timer " << this << " >> waiting " << wait_time
                 << " miliseconds for next update.");
  }
