...
protected:
  ...
  string m_window_name; // the window name to send the input to. the window that
                        // contains this name will be selected.
  string m_return_window_name; // the window name to return to at the end of
                               // send().
  int m_max_windows; // the maximum number of windows that the activate_window()
                     // method will look through. This is to avoid infinite alt
                     // tabbing.
  queue<string> m_message_queue; // message queue to send multiple messages in
                                 // one alt tab operation.
...
public:
   ...
   * string s: the string that is to be queued. (*WARNING*: read method
   * send_string() for details.)
   */
  void queue_message(string s) {
    LOG("MS_Window " << this << " >> queue_message(), Window: " << m_window_name
                     << ", String: " << s);
    m_message_queue.push(s);
  }

  /**
   * Attempts to send strings that are queued as keyboard input to the window
   * that contains "m_window_name" in its title. Will attempt change the window
   * to paycheck.exe after. bool return value: returns false if failed.
   * otherwise returns true
   */
  bool send() {
    if (m_message_queue.empty())
      return true; // do nothing if queue is empty.
    ...
    if (ret) { // if success.
      ...
      LOG("MS_Window " << this
                       << " >> send() not found, Window: " << m_window_name);
    }
...
protected:
   ...
   * string s: The string to be send as keyboard input. (Can only accpets a
   * handful of inputs.)
   */
  void send_string(string s) {
    for (unsigned int i = 0; i < s.length() + 1; i++) {
      if ('a' <= s[i] && 'z' >= s[i]) {
        ...
        LOG("MS_Window " << this << " >> send_string(): invalid character: "
                         << s[i] << ". skipped.");
      }
   ...
   * string return value: the name of the forground window. Will return a empty
   * string if failed.
   */
  ...
  /**
   * Will attempt to alt tab to the window with the title that contains the
   * given string. string windowTitle: the window you will search for. bool
   * return value: will return false if failed. otherwise will return true.
   */
...
/**
 * A version of MS_Window that will send messages into sub-window of windows
 * using Ctrl-Tab.
 */
...
public:
  MS_Window_CT(int delay, int maxW, string WName, string ret_WName,
               string sub_window_name)
      : MS_Window(delay, maxW, WName, ret_WName) {
  ...
  bool send() {
    if (m_message_queue.empty())
      return true; // do nothing if queue is empty.
    LOG("MS_Window_CT " << this << " >> send(), Window: " << m_window_name
                        << ", Sub window: " << m_sub_window_name);
    bool ret = activate_window(m_window_name);
    if (ret)
      ret = activate_sub_window();
    if (ret) { // if success.
      while (!m_message_queue.empty()) {
        Sleep(m_input_delay);
        send_string(m_message_queue.front());
