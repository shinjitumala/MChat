 ...
 */
class Schedule {
  bool schedule_table[7]
                     [24 * QUANTUM_NUMBER]; // if this is set to true it means
                                            // that the Message_Handler will
  ...
  Schedule() {
    for (int i = 0; i < 7; i++) {
      for (int j = 0; j < 24 * QUANTUM_NUMBER; j++) {
        schedule_table[i][j] =
            false; // default is false. meaning no sending messages.
      }
    }
...
private:
  void check(int week, int time_frame) {
    if (week < 0 || 6 < week || time_frame < 0 ||
        24 * QUANTUM_NUMBER - 1 < time_frame) {
      LOG("Schedule " << this
                      << " >> check(): argument was out of range. week: "
                      << week << ", time_frame: " << time_frame);
      exit(1);
    }
  }
};
...
protected:
  ...
  int m_interval_min; // minimum message interval. in milliseconds. 300000 for 5
                      // mins.
  int m_interval_max; // maximum message interval. in milliseconds. 300000 for 5
                      // mins.
...
public:
  ...
  /**
   * Returns a double value that indicates the progress to the next message
   * send. double return: progress of the message sending.
   */
  double get_progress() {
    if (m_next_counter == 0)
      return 1;
    return m_counter / m_next_counter;
  }

protected:
  /**
   * Updates the current counter. And will return true if it is time to queue a
   * message. tm *time: current time. bool return: will return true if it is
   * time to queue a message.
   */
  bool check_interval(tm *time) {
    ...
    m_counter +=
        d_day * 86400000 + d_hour * 3600000 + d_min * 60000 + d_sec * 1000;
  ...
  void set_next_counter() {
    m_next_counter =
        m_interval_min + m_generator() % (m_interval_max - m_interval_min);
    LOG("Word_Handler " << this
                        << " >> set_next_counter(): " << m_next_counter);
  }
};
...
public:
   ...
   * Constructor
   */
  Word_Handler(Schedule *schedule, Message_Sender *ms, string message,
               int interval_min, int interval_max) {
    m_schedule = schedule;
    m_ms = ms;
    m_message = message;
    ...
    set_next_counter();
    LOG("Word_Handler " << this << " >> new. Schedule: " << schedule
                        << ", Message_Sender: " << ms);
  }
   ...
   * Deconstuctor
   */
  ~Word_Handler() { LOG("Word_Handler " << this << " >> delete."); };
  ...
  void update(tm *time) {
    ...
    if ((*m_schedule)
            .get_schedule(time->tm_wday,
                          QUANTUM_NUMBER * time->tm_hour +
                              time->tm_min / (60 / QUANTUM_NUMBER))) {
...
public:
   ...
   * Constructor
   */
  Markov_Generator(Schedule *schedule, Message_Sender *ms, bool is_dictionary,
                   string input_file_path, int interval_min, int interval_max) {
    m_schedule = schedule;
    m_ms = ms;
    ...
    m_previous = *localtime(&now);
    set_next_counter();
    LOG("Markov_Generator " << this << " >> new. Schedule: " << schedule
                            << ", Message_Sender: " << ms);
  }
